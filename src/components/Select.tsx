...
type ThemeSelectProps = SelectProps<ThemeDefinition>;
type LanguageSelectProps = SelectProps<LanguageDefinition>;
type FontSelectProps = SelectProps<FontDefinition>;

type SelectPropsUnion =
  | ThemeSelectProps
  | LanguageSelectProps
  | FontSelectProps;

export default memo(function Select(props: SelectPropsUnion) {
  ...
  return (
    <Listbox value={initialValue} onChange={setValue}>
      <div className="relative">
        <Listbox.Button
          className={clsx(
            "flex w-auto items-center, justify-between gap-3 select-none rounded-lg p-2 text-sm",
